...
const App = {
    ...
    initHeader() {
        ...
        headerEl.innerHTML = `
            <div class="container py-6 flex justify-between items-center">
                <a href="index.html" class="logo-text">MERMAID HOMES</a>
                <nav class="flex items-center space-x-8 text-sm font-light tracking-wider">
                    <a href="listings.html?type=buy" class="nav-link">Buy</a>
                    <a href="listings.html?type=rent" class="nav-link">Rent</a>
                </nav>
            </div>
        `;
    ...
    async initListingsPage() {
            ...
            <div class="container">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                    <div class="md:col-span-2">
                        <h4 class="logo-text">MERMAID HOMES</h4>
                        <p class="text-sm text-gray-400 max-w-md">Your trusted real estate partner.</p>
                    </div>
                    <div>
                        <h4>Quick Links</h4>
                        <ul class="space-y-3 text-sm">
                            <li class="text-gray-400 hover:text-white"><a href="listings.html?type=buy">Buy</a></li>
                            <li class="text-gray-400 hover:text-white"><a href="listings.html?type=rent">Rent</a></li>
                        </ul>
                    </div>
                    <div>
                        <h4>Follow Us</h4>
                        <div class="flex space-x-5 text-2xl">
                            <a href="https://www.facebook.com/profile.php?id=61551898026585" target="_blank" class="text-gray-400 hover:text-white"><i class="fab fa-facebook"></i></a>
                            <a href="https://www.instagram.com/merm.homes" target="_blank" class="text-gray-400 hover:text-white"><i class="fab fa-instagram"></i></a>
                            <a href="https://wa.me/66654765114" target="_blank" class="text-gray-400 hover:text-white"><i class="fab fa-whatsapp"></i></a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        document.getElementById('add-listing-modal').innerHTML = `
            <div class="modal-content">
                ...
                <form id="property-form" class="space-y-4">
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <div>
                            <label>Beds</label>
                            <input type="number" name="bedrooms" required class="w-full p-2 border rounded" min="0">
                        </div>
                        <div>
                            <label>Baths</label>
                            <input type="number" name="bathrooms" required class="w-full p-2 border rounded" min="0">
                        </div>
                        <div>
                            <label>Area (m²)</label>
                            <input type="number" name="area" required class="w-full p-2 border rounded" min="0">
                        </div>
                    </div>
                    <div>
                        <label>Price (THB)</label>
                        <input type="number" name="priceTHB" required class="w-full p-2 border rounded" min="1">
                    </div>
                    <div>
                        <label>Image URL</label>
                        <input type="url" name="images" required class="w-full p-2 border rounded" placeholder="https://...">
                    </div>
                    <button type="submit" class="w-full bg-[var(--primary-color)] text-white p-3 rounded-lg !mt-6 flex items-center justify-center">Add Property</button>
                </form>
            </div>
        `;
    ...
    renderListings() {
        ...
        if (!grid) return;
        const data = this.propertiesData.filter(p => p.type === this.currentListingType);
        ...
        grid?.addEventListener('click', async e => {
            ...
            if (btn && confirm('Are you sure?')) {
                e.stopPropagation();
                await this.deleteProperty(btn.dataset.id);
            }
        });
    },
    ...
    renderPropertyCard(property) {
        ...
        return `
            <div class="property-card relative">
                <button class="delete-property-btn" data-id="${property.id}"><i class="fas fa-trash-alt"></i></button>
                <div class="relative h-56 bg-gray-200">
                    <img src="${imageUrl}" onerror="this.src='fallback.jpg'" class="w-full h-full object-cover">
                </div>
                <div class="p-4">
                    <h4 class="font-semibold truncate text-gray-800">${property.title}</h4>
                    <p class="text-sm text-gray-500 my-1"><i class="fas fa-map-marker-alt mr-2"></i>${property.location}</p>
                    <div class="mt-4 font-semibold text-lg text-gray-900">${price}</div>
                </div>
            </div>
        `;
    },
